...
;(function($, window, document,undefined) {
	...
	// 验证码加载占位图（data-URI，请求返回前显示，避免空白；不依赖外部文件）
	var CAPTCHA_PLACEHOLDER = "data:image/svg+xml,%3Csvg%20xmlns='http://www.w3.org/2000/svg'%20width='310'%20height='155'%3E%3Crect%20fill='%23e9ecef'%20width='310'%20height='155'/%3E%3Ctext%20fill='%239e9e9e'%20x='50%25'%20y='50%25'%20text-anchor='middle'%20dy='.3em'%20font-size='14'%3E%E5%8A%A0%E8%BD%BD%E4%B8%AD...%3C/text%3E%3C/svg%3E";
    ...
    function getPictrue(data,baseUrl,resolve,reject){
		baseUrl = baseUrl.replace(/\/+$/, '');   // 去除结尾 / 防止拼接出 //get.php
		$.ajax({
			type : "post",
	...
	function checkPictrue(data,baseUrl,resolve,reject){
		baseUrl = baseUrl.replace(/\/+$/, '');   // 去除结尾 / 防止拼接出 //check.php
		$.ajax({
			type : "post",
			contentType: "application/x-www-form-urlencoded;charset=UTF-8",
    ...
    Slide.prototype = {
        init: function() {
        	...
        	//加载页面
        	this.loadDom();
        	// 弹出式(pop)模式下不在初始化时拉取验证码图片，
        	// 避免一进页面就向后端发 get.php 请求；等用户点击触发按钮弹框显示时再请求
        ...
        loadDom : function() {
			...
			panelHtml += '<div class="verify-img-out">'+
							'<div class="verify-img-panel">'+
								...
								'<img src="'+CAPTCHA_PLACEHOLDER+'" class="backImg" style="width:100%;height:100%;display:block">'+
									...
									'<div class="verify-left-bar">'+
										...
										'<div  class="verify-move-block">'+
											...
											'<div class="verify-sub-block">'+
												'<img src="'+CAPTCHA_PLACEHOLDER+'" class="bock-backImg" alt=""  style="width:100%;height:100%;display:block">'+
				...
				checkPictrue(data,this.options.baseUrl,function(res){
					...
					if (res.repCode=="0000") {
						...
						setTimeout(function(){
							...
							// 验证成功后不再自动刷新，避免重复向 get.php 发请求；
							// 下次点击下载按钮打开弹框时 refresh() 会重置状态并拉取新验证码
						},1000)
        ...
        refresh: function() {
        	var _this = this;
        	// 重新请求前先显示占位图，避免仍显示上一张/空白
        	this.$element.find(".backImg")[0].src = CAPTCHA_PLACEHOLDER;
        	this.$element.find(".bock-backImg")[0].src = CAPTCHA_PLACEHOLDER;
			...
			getPictrue({captchaType:"blockPuzzle", clientUid: localStorage.getItem('slider'), ts: Date.now()},this.options.baseUrl,function (res) {
				...
				if (res.repCode=="0000") {
					...
					_this.$element.find(".backImg")[0].src = CAPTCHA_PLACEHOLDER
					_this.$element.find(".bock-backImg")[0].src = CAPTCHA_PLACEHOLDER
        ...
        refresh: function() {
			...
			getPictrue({captchaType:"clickWord", clientUid: localStorage.getItem('point'), ts: Date.now()},_this.options.baseUrl,function(res){
				...
				if (res.repCode=="0000") {
					...
					_this.htmlDoms.back_img[0].src = CAPTCHA_PLACEHOLDER;
